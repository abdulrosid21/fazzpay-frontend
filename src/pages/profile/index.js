...
import { getDataUserById, updateImage } from "../../redux/actions/user";
...
export default function Profile() {
  ...
  const handleUpdateImage = () => {
    ...
    dispatch(updateImage({ userId: userData.id, image: imageData })).then(
      () => {
        setToast(true);
      }
    );
    ...
    <Layout title="Profile" page={"Profile"}>
      ...
      <div className="bg-white rounded-3xl shadow-lg px-7 pt-12 pb-16">
        <div className="grid xl:justify-center text-center">
          <div className="w-20 h-20 rounded-xl mx-auto mb-3 overflow-hidden flex">
            <Image
              className="w-20 h-20 justify-center m-auto bg-cover bg-center rounded-lg"
              src={
                lengthImage > 0
                  ? imagePreview
                  : userData.image
